...
The Linux half of the stack: hold **Right Alt**, speak, release — whisper
...
1. **voxtype itself** — not installed by this script:
...
2. **The `input` group** — the evdev hotkey reads `/dev/input/*` directly:

   ```sh
   sudo usermod -aG input "$USER"   # then log out and back in
   ```

3. **llama.cpp with a CPU backend** — see the trap below.
...
1. **Preflight** — reports missing voxtype / llama-server, the `input` group,
   and runs the `VOXTYPE_CONTEXT` name check (see
   [troubleshooting](troubleshooting.md#voxtype_context-is-no-longer-yours)).
2. **Payload** — `voxtype-cleanup`, `voxtype-meeting`, `voxtype-notify`,
...
3. **The cleanup server** — renders `llama-server.service.in` into
   ...
   (or installs the unit *not started* while the model is missing — a
...
5. **Reports** — the multi-GB model downloads (never performed) and the
...
flip their order — llama-server would then come up happily on the slow one,
and the only symptom would be "dictation cleanup got slow". The installer
...
- `--temp 0` — this is copy-editing under a never-add-a-word rule; sampling
...
start/stop but no toggle), with `export` and `summarize` subcommands
(summaries are local by default — map-reduce over ~2,000-token chunks through
the same llama-server; cloud backends are an explicit opt-in, never a
fallback).
...
- **Hyprland** (`hyprland-bind.lua`): a `SUPER + CTRL + M` bind running
  `voxtype-meeting toggle` — written for Omarchy's hyprlua bindings file, so
  copy the line into `~/.config/hypr/bindings.lua`.
